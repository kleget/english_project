...
def removing_anomaly(mas):  # куча всяких обработок, просто чистим данные полученные из книг
    mas = lemmatize_text_en(lemmatize_text_ru(mas))  # удаляем аномалии из текста  #'это работает очень медленно, 70 секунд на войну и мир
    mas = mas.split()
...
def analysand_func_dict(name_file):  # возвращает список слов в виде словаря
    ...
    # что именно работает медленно, код ниже или get_txt_file из-за (text = removing_anomaly(f))
